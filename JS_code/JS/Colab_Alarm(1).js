	...
	detect = setInterval(
		function (){
			...
			if(fav.indexOf('busy')==-1){
				sound.src = fav.indexOf('error')>0?path+'warn.mp3':path+'movie.mp3';
				sound.play();
				setTimeout(function(){
				...
				if(document.getElementById('alarm-btn')==null){
					n=document.createElement('colab-toolbar-button');
					n.id='alarm-btn';n.innerText='Alarm';
					n.title='Click there to set an Alarm for long-time process running';
					n.onclick=setAlarm;
					document.getElementById('top-toolbar').insertBefore(n,document.getElementsByTagName('colab-connect-button')[0]);
				}
setAlarm(a=false);void(0);